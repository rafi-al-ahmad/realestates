...
class PropertiesController extends Controller
{
    ...
    public function propertiesValidationRules($rules = [])
    {
        return array_merge([
            ...
            'property_type' => ['nullable', Rule::exists('definitions', 'id')->where('status', 1)->where('type', 'property_type')],
            'category_id' => ['required', Rule::exists('categories', 'id')->where('status', 1)],
            'price_tl' => ['required', 'numeric'],
            'price_usd' => ['required', 'numeric'],
            'interchange' => ['nullable', Rule::in([0, 1])],
            'is_furnished' => ['nullable', Rule::in([0, 1])],
            'is_featured' => ['nullable', Rule::in([0, 1])],
            'credit_eligibility' => ['nullable', Rule::in([0, 1])],
            'housing_type' => ['nullable', Rule::exists('definitions', 'id')->where('status', 1)->where('type', 'housing_type')],
            'heating_type' => ['nullable', Rule::exists('definitions', 'id')->where('status', 1)->where('type', 'heating_type')],
            'building_age' => ['nullable', Rule::exists('definitions', 'id')->where('status', 1)->where('type', 'building_age')],
            'user_status' => ['nullable', Rule::exists('definitions', 'id')->where('status', 1)->where('type', 'user_status')],
            ...
            'net_area' => ['nullable', 'numeric'],
            'gross_area' => ['nullable', 'numeric'],
            'living_rooms_no' => ['nullable', 'numeric'],
            'bedrooms_no' => ['nullable', 'numeric'],
            'bathrooms_no' => ['nullable', 'numeric'],
            'building_floors' => ['nullable', 'numeric'],
            'floor_number' => ['nullable', 'numeric'],
            'photos' => ['nullable', 'array'],
            'photos.*' => ['nullable', 'file', 'max:2400', 'image'],
        ], $rules);
    }
    ...
    public function store(Request $request)
    {
        ...
        $property->credit_eligibility = $data['credit_eligibility']  ?? null;
        $property->interchange = $data['interchange']  ?? null;
        $property->housing_type_id = $data['housing_type']  ?? null;
        $property->building_age_id = $data['building_age']  ?? null;
        $property->heating_type_id = $data['heating_type']  ?? null;
        $property->rent_deposit = $data['rent_deposit'] ?? null;
        $property->rental_income = $data['rental_income'] ?? null;
        $property->dues = $data['dues'] ?? null;
        $property->net_area = $data['net_area']  ?? null;
        $property->gross_area = $data['gross_area']  ?? null;
        $property->living_rooms_no = $data['living_rooms_no']  ?? null;
        $property->bedrooms_no = $data['bedrooms_no']  ?? null;
        $property->bathrooms_no = $data['bathrooms_no']  ?? null;
        $property->building_floors = $data['building_floors']  ?? null;
        $property->floor_number = $data['floor_number']  ?? null;
        $property->user_status_id = $data['user_status']  ?? null;
        $property->property_type_id = $data['property_type']  ?? null;
        $property->category_id = $data['category_id']  ?? null;
        $property->agent_id = $data['agent_id']  ?? null;
